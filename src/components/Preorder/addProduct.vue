...
const allProducts = computed(() => {
  // const prod = firstProducts.find(e => e.id = EditProduct.id).length > 0  ? products.filter(e => e.product.id != EditProduct.id) : products;
...
const calcularCaja = computed(() => {
  let data = verificarPrecioCaja(allProducts.value, EditProduct)
  return data
})
...
const selectPrice = computed(() => {
  ...
  if (order.client.rate.id <= 3) {
    if ((insertPro.amount_require >= EditProduct.pieces && (unit_measure.val.id == 1 || unit_measure.val.id == 2)) || unit_measure.val.id == 3 || calcularCaja.value) {
      return 4;
...
const actPrice = () => {
  ...
  allProducts.value.forEach(e => {
    ...
    const cumpleCaja = verificarPrecioCaja(allProducts.value, e.product)
    if (cumpleCaja) {
      e.price = e.product.prices.filter(i => i._rate == 4)[0].price
      e.rates = e.product.prices.filter(i => i._rate == 4)[0].rates
      e.total = e.product.prices.filter(i => i._rate == 4)[0].price * e.amount_require
    } else {
      e.price = e.product.prices.filter(i => i._rate == 1)[0].price
      e.rates = e.product.prices.filter(i => i._rate == 1)[0].rates
      e.total = e.product.prices.filter(i => i._rate == 1)[0].price * e.amount_require
    }
    if (cumpleDocena) {
      if (e.rates.id < 3) {
...
const verificarPrecioMayoreo = (prdts, product) => {
  ...
  if (inx >= 0) {
    if (EditProduct.id === product.id) {
      // prdts = firstProducts.length > 0 ? prdts : prdts.filter(e => e.product.id !== product.id)
