  ...
  <q-page padding>
    ...
    <q-card>
      <q-card-section class="items-center">
        <q-input v-model="addDevice.ip" type="text" label="IP" filled :error="texterro"
          error-message="La direccion Ip ya esta esta en uso favor de intentar con otra">
          <template v-slot:append>
            <q-btn color="primary" icon="sensors" @click="ping" flat :disable="addDevice.ip == '' ? true : false"
              v-if="!texterro" />
          </template>
        </q-input>
        ...
        <div v-if="addDevice.state">
          ...
          <q-separator spaced inset vertical dark />
          <q-select v-model="addDevice._store" :options="mosStor" label="Sucursal" filled option-label="name"
            option-value="id" />
          <q-separator spaced inset vertical dark />
