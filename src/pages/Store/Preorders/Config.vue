      ...
      <q-card class="bg-darkl1">
        ...
        <q-list class="text-bold" separator>
          <q-item tag="label" v-ripple v-for="config in cnf" :key="config._state">
            ...
            <q-item-section side>
              <q-toggle v-model="config.active" color="primary" :true-value="1" :false-value="0" keep-color
                @update:model-value="actState(config)" size="md" />
            </q-item-section>
          </q-item>
        </q-list>
      </q-card>
...
import { $sktpvt, $socket } from 'boot/socket'
