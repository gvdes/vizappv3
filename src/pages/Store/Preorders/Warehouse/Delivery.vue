...
const message = computed(() => configs.value.filter(c => c._state_order == 4)[0]?.active)
const isMobile = computed(() => $q.platform.is.mobile);
const bascketOrder = computed(() => orders.value.filter(e => e.state.id == 4))
...
const init = async () => {
  ...
  if (resp.error) {
    ...
    console.log(resp)
    configs.value = resp
    let act = configs.value.filter(c => c._state_order == 4)[0]?.active
    if (act == 1) {
      getOrders();
    }
  }
